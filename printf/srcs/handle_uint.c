...
/*   Updated: 2017/02/20 21:15:15 by ekelen           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			handle_uint(uintmax_t arg_uint, t_arg *tg)
{
	...
	get_uint_lenmod(tg);
	if (!ft_strcmp(LN, "l"))
		arg_uint_str = ft_itoa_base((unsigned long int)arg_uint, tg->base, tg);
	else if (!ft_strcmp(LN, "ll"))
		arg_uint_str = ft_itoa_base((unsigned long long)arg_uint, tg->base, tg);
	else if (!ft_strcmp(LN, "h"))
		arg_uint_str = ft_itoa_base((unsigned short)arg_uint, tg->base, tg);
	if (!ft_strcmp(LN, "hh"))
		arg_uint_str = ft_itoa_base((unsigned char)arg_uint, tg->base, tg);
	if (!ft_strcmp(LN, "z"))
		arg_uint_str = ft_itoa_base((size_t)arg_uint, tg->base, tg);
	if (!ft_strcmp(LN, "j"))
		arg_uint_str = ft_itoa_base((uintmax_t)arg_uint, tg->base, tg);
	if (!LN[0])
		arg_uint_str = ft_itoa_base((unsigned int)arg_uint, tg->base, tg);
