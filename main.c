...
int		new_entry(struct stat stp, char *path, struct dirent *dp, t_opt *e)
{
	int sort_mode = 2;
	static int which = 0;
	if (!(path || dp || e->tree))
	...
	new_tree(new, &(e->tree), sort_mode);
	return (1);
}


int		rec_check(char *s, t_opt *e)
{
	DIR 				*dir = NULL;
    struct dirent		*dp;
    struct stat			stp;
	...
	while ((dp = readdir(dir)) != NULL)
    {
		...
		else
		{
			...
			{
				lstat(path, &ltp);
				new_entry(ltp, path, dp, e);
			}
			else
				new_entry(stp, path, dp, e);
		}
