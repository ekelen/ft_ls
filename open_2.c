...
static void	open_subdir(t_opt *e, t_dir cwd, t_ls *entry, int first) // after printing one dir, call recursion
{
	if (!entry)
		return ;
	if (entry->left)
...
static void	open_subdir_rev(t_opt *e, t_dir cwd, t_ls *entry, int first) // after printing one dir, call recursion
{
	if (!entry)
		return ;
	if (entry->right)
